...
from libs.parallel import EvaluatorParallel
...
from arguments.ns_neat import get_args


def main():
    args = get_args()

    save_path = os.path.join('out', 'ns_neat', args.name)
    ...
    config_file = os.path.join('config', 'ns_neat.cfg')
    custom_config = [
    ...
    config_out_file = os.path.join(save_path, 'ns_neat.cfg')
